...
class categories {
    ...
    public function add($db) {
        ...
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query->error =  "error at add new categories : ".$stmt->error;
    ...
    public function one($db) {
        $result_query = new result_query();
        $one = new categories();
        $query = "SELECT id,name,image_url FROM categories WHERE id=? LIMIT 1";
        $stmt = $db->prepare($query);
        $stmt->bind_param('i', $this->id);
        $stmt->execute();      
        if ($stmt->error != ""){
            $result_query-> error = "error at query one categories: ".$stmt->error;
            $stmt->close();
            return $result_query;
        }
    ...
    public function all($db,$list_query) {
        ...
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query-> error = "error at query all categories : ".$stmt->error;
            $stmt->close();
            return $result_query;
        }
        $rows = $stmt->get_result();
    ...
    public function update($db) {
        ...
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query->error = "error at update one categories : ".$stmt->error;
            $result_query->data = "not ok";
            $stmt->close();
            return $result_query;
        }
    ...
    public function delete($db) {
        ...
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query->error = "error at delete one categories : ".$stmt->error;
            $result_query->data = "not ok";
            $stmt->close();
            return $result_query;
        }
