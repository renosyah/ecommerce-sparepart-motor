...
class users {
    ...
    public function add($db) {
        ...
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query->error =  "error at add new users : ".$stmt->error;
    ...
    public function one($db) {
        $result_query = new result_query();
        $one = new users();
        $query = "SELECT id,name,username,password FROM users WHERE id=? LIMIT 1";
        $stmt = $db->prepare($query);
        $stmt->bind_param('i', $this->id);
        $stmt->execute();      
        if ($stmt->error != ""){
            $result_query-> error = "error at query one users: ".$stmt->error;
            $stmt->close();
            return $result_query;
        }
    ...
    public function login($db) {
        $result_query = new result_query();
        $one = new users();
        $query = "SELECT id,name,username,password FROM users WHERE username=? LIMIT 1";
        $stmt = $db->prepare($query);
    ...
    public function update($db) {
        ...
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query->error = "error at update one users : ".$stmt->error;
            $result_query->data = "not ok";
            $stmt->close();
            return $result_query;
        }
    ...
    public function delete($db) {
        ...
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query->error = "error at delete one users : ".$stmt->error;
            $result_query->data = "not ok";
            $stmt->close();
            return $result_query;
        }
