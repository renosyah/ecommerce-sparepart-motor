...
class payments {
    ...
    public function add($db) {
        ...
        $query = "INSERT INTO payments (name,detail) VALUES (?,?)";
        $stmt = $db->prepare($query);
        $stmt->bind_param('ss', $this->name, $this->detail);
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query->error =  "error at add new payments : ".$stmt->error;
    ...
    public function one($db) {
        $result_query = new result_query();
        $one = new payments();
        $query = "SELECT id,name,detail FROM payments WHERE id=? LIMIT 1";
        $stmt = $db->prepare($query);
        $stmt->bind_param('i', $this->id);
        $stmt->execute();      
        if ($stmt->error != ""){
            $result_query-> error = "error at query one payments: ".$stmt->error;
            $stmt->close();
            return $result_query;
        }
    ...
    public function all($db,$list_query) {
        ...
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query-> error = "error at query all payments : ".$stmt->error;
            $stmt->close();
            return $result_query;
        }
        $rows = $stmt->get_result();
    ...
    public function update($db) {
        $result_query = new result_query();
        $result_query->data = "ok";
        $query = "UPDATE payments SET name = ?,detail = ? WHERE id=?";
        $stmt = $db->prepare($query);
        $stmt->bind_param('ssi', $this->name, $this->detail,$this->id);
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query->error = "error at update one payments : ".$stmt->error;
            $result_query->data = "not ok";
            $stmt->close();
            return $result_query;
        }
    ...
    public function delete($db) {
        ...
        $stmt->execute();
        if ($stmt->error != ""){
            $result_query->error = "error at delete one payments : ".$stmt->error;
            $result_query->data = "not ok";
            $stmt->close();
            return $result_query;
        }
